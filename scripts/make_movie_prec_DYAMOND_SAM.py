...
def loadPrec(i_t):
    ...
    precac_prev = loadPrecac(i_t-1)
    precac_current = loadPrecac(i_t)
...
def importData(i_t):
    ...
    # Load DYAMOND data
    Prec_DYAMOND = loadPrec(i_t)

    # # Load TOOCAN data
    # img_TOOCAN = xr.open_dataarray(path_TOOCAN)

    return Prec_DYAMOND#, img_TOOCAN
...
def showColorBar(fig,ax,im):
    ...
    cbar.ax.set_ylabel('30-mn Precipitation (mm)')

def initFigure(i_t0,Lx_fig,Ly_fig,title=None,norm=None):
    
    # load data
    prec = importData(i_t0)
    ...
    for slice_lon in slice(lon_lim[0],360),slice(0,lon_lim[1]):
        ...
        # coords
        lonarray_dyamond,latarray_dyamond = getCoords2D(prec,slice_lon,slice_lat)            
        # data
        Z = prec.sel(lon=slice_lon,lat=slice_lat)
        # log colorbar
...
def makeMovie(movie_path,j_start,j_end,Lx_fig,Ly_fig):
    ...
    def updateImage(i_t):
        ...
        for slice_lon,i_ims in zip([slice(lon_lim[0],360),slice(0,lon_lim[1])],list(np.arange(2))):
            ...
            # update images
            ims[i_ims].set_array(np.ravel(Z[:-1,:-1].data))
